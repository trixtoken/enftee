...
(function($){
	...
	function drawSin(xOffset,color1,color2){
		...
		for(var x = w_sX;x < w_sX + w_axisLength;x += 20 / w_axisLength) {
			...
			// 2021-01-05 var dY = c_height * (1 - config.nowRange / 100 );
			var dY = c_height * (1 - config.nowRange / 50 );
		...
		if (!config.isLoading) {
			ctx.save();
			// 2021-01-05 var size = 0.4 * config.circle_config.cR;
			var size = 0.5 * config.circle_config.cR;
			ctx.font = size + 'px Microsoft Yahei';
			ctx.textAlign = 'center';
			ctx.fillStyle = config.textColorRange[getIndex.call(this)];
			// 2021-01-05 ctx.fillText(~~config.nowRange + '%', config.circle_config.r, config.circle_config.r + size / 2);
