...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'member/prescription_view.tpl',
...
$prescription = sql_fetch("select p.*, m.mb_name, pet.pe_name, pet.pe_type1, pet.pe_birth, q.pq_idx, q.pq_contents, q.pq_answer, q.pq_img from rb_prescription as p left join rb_pet as pet on p.pe_idx = pet.pe_idx left join rb_member as m on p.mb_id = m.mb_id left join rb_prescription_question as q on p.pr_idx = q.pr_idx where p.vet_id = '".$member['mb_id']."' and p.pr_idx = '$pr_idx'");
