        ...
        */
        ...
        private function StartElement($parser, $name, $attrs = array())
        {
            ...
            //If it isn't root level, use the stack to find the parent
                ...
                $parent->AddChild($name, $attrs, custom_count($this->stack), $this->cleanTagNames);
...
class XMLTag
{
    ...
    public function Delete($childName, $childIndex = 0)
    {
        ...
        for($x = 0; $x < custom_count($this->tagChildren); $x ++)
        {
...
function loadXML2($domain, $path, $timeout = 30) {
         ...
         if($fp) {
                  ...
                  // check status is 200
				  //$resp = mb_convert_encoding(urldecode($resp),'EUC-KR','UTF-8');
                  ...
                  if(preg_match($status_regex, $resp, $matches) && $matches[1] == 200) {
						   ...
						   $temp_pos = strpos($temp[1], "<?xml");
						   $parts = substr($temp[1], $temp_pos);



                           //return simplexml_load_string($parts[1], 'SimpleXMLElement',LIBXML_NOWARNING|LIBXML_NOERROR);
						   //$parts = mb_convert_encoding(urldecode($parts),'EUC-KR','UTF-8');
						   return $parts;
...
function get_map_position($address, $key)
{
	global $_cfg;
	/*
	$rslt = array();
	...
	*/
	...
	$simple_xml = loadXML2("openapi.map.naver.com", "/api/geocode.php?key=".$key."&encoding=utf-8&coord=latlng&query=".$address);
	//$simple_xml = loadXML2("www.anovein.com", "/sample.xml");

	$parser = new XMLParser($simple_xml);

	$parser->Parse();
	//print_r($parser->document);
	$x_pos = $parser->document->item[0]->point[0]->x[0]->tagData;

	$y_pos = $parser->document->item[0]->point[0]->y[0]->tagData;

	$rslt = array();
	$rslt['x_pos'] = ($x_pos) ? $x_pos : 0;
	$rslt['y_pos'] = ($y_pos) ? $y_pos : 0;
	//print_r($rslt);
	return $rslt;
}

function get_map_position_daum($address, $key)
{
	global $_cfg;
	/*
	$rslt = array();
	...
	*/
	...
	$simple_xml = loadXML2("apis.daum.net", "/local/geo/addr2coord?apikey=".$key."&q=".$address."&output=xml");
	//$simple_xml = loadXML2("www.anovein.com", "/sample.xml");
	...
	$parser->Parse();
	//print_r($parser->document);
	$x_pos = $parser->document->tagChildren[8]->tagChildren[14]->tagData;

	$y_pos = $parser->document->tagChildren[8]->tagChildren[13]->tagData;
