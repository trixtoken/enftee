...
class Template_
{
	...
	*/
	...
	//메모리에 있는것 fetch해서 뿌리기
	function memory_fetch($contents)
	{
		$te_dir =  $this->template_dir;
		$co_dir =  $this->compile_dir;
		$ca_dir =  $this->cache_dir;

		$tmp_dir = "/".$this->skin."/".$this->memory_tmp;

		$te_tmp = $te_dir.$tmp_dir;
		$co_tmp = $co_dir.$tmp_dir;
		$ca_tmp = $ca_dir.$tmp_dir;
		...
		$fp = fopen($te_tmp."/".$new_id.".tpl", 'w');
		fwrite($fp, $contents);
		fclose($fp);

		$this->_define($new_id, $this->memory_tmp.'/'.$new_id.'.tpl');
		$rslt = $this->fetch($new_id);

		@unlink($te_tmp."/".$new_id.".tpl");
		@unlink($co_tmp."/".$new_id.".tpl");
		@unlink($ca_tmp."/".$new_id.".tpl");
