...
class Template_Compiler_
{
	function _compile_template($tpl, $tpl_path, $cpl_base, $cpl_head)
	{
		...
		$this->safe_mode_func     =$tpl->safe_mode_func;
	...
	// get safe mode functions
		if ($this->safe_mode || $this->safe_mode_func) {
	...
	function _compile_expression($expression, $escape=0, $no_directive=0)
	{
		...
		for ($xpr='',$i=0; strlen($expression); $expression=substr($expression, strlen($m[0])),$i++) {	// 
			...
			} elseif ($m[2] || '0'===$m[2]) {		// ([A-Z_a-z\x7f-\xff][\w\x7f-\xff]*)
				if (empty($m[3])) $m[3]='';		// (\[|\.|\(|\-\>)
				switch ($m[3]) {
				case ''  :
					switch ($var_state[0]) {
				...
				case '(' :
					if ($var_state[0]) {
						...
						if (in_array($func, $this->func_plugins)) {
							...
							if ($this->safe_mode || $this->safe_mode_func) in_array($func, $this->safe_mode_functions) or $this->exp_error[]=array(5, $m[2]);
