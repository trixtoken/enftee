...
// 전체 데이터수 구하기
$sql_total = "select * from rb_product_review as q where q.pd_idx = '$pd_idx' $search_query";
...
if($total){
	...
	$sql = "select * from rb_product_review as q where q.pd_idx = '$pd_idx' $search_query $order_query $limit_query";
	$data = sql_list($sql);

	for($i=0;$i<count($data);$i++){
		$data[$i][pr_title_v] = get_text($data[$i][pr_title]);
		$data[$i][pr_contents_v] = conv_content($data[$i][pr_contents], '');
	}
