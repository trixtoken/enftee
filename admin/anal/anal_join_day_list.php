...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'anal/anal_join_day_list.tpl',
	'body'  =>'inc/body.tpl',
	'top'  =>'inc/top.tpl',
...
$s_month = ($s_month) ? $s_month : date("n");
$s_str = $s_year."-".make_zero_first($s_month, 2);

$querys = array();
$querys_page = array();

$search_query = " and SUBSTRING(mb_regdate, 1, 7) >= '$s_str' ";
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_member  where 1 $search_query";
$total = sql_total($sql_total);

$sql = " select count(*) as cnt, SUBSTRING(mb_regdate, 1, 10) as da
          from rb_member  where 1
          $sql_search
		  group by SUBSTRING(mb_regdate, 1, 10)
...
$data = array();
for ($i=1; $i<=date("t", strtotime($s_str)); $i++) {
