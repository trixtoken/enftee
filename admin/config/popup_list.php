...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'config/popup_list.tpl',
	'body'  =>'inc/body.tpl',
	'top'  =>'inc/top.tpl',
...
if($_GET[pp_agent]){
	$search_query .= " and m.pp_agent = '$_GET[pp_agent]' ";
}
$querys[] = "pp_agent=".$_GET[pp_agent];
$querys_page[] = "pp_agent=".$_GET[pp_agent];


$query_order = (is_array($querys) && count($querys) > 0) ? implode("&", $querys) : "";
$order_query = "order by m.pp_idx asc";
if($order_by != ""){
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_popup as m where 1 $search_query";
...
if($total){
	...
	$sql = "select * from rb_popup as m where 1 $search_query $order_query $limit_query";
	$data = sql_list($sql);
