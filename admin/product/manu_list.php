...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'product/manu_list.tpl',
	'body'  =>'inc/body.tpl',
	'top'  =>'inc/top.tpl',
...
if($_GET[sca] && $_GET[stx]){
	switch($_GET[sca]){
		case "bd" : 
			$search_query .= " and (p.mn_name like '%$_GET[stx]%' or p.mn_contents like '%$_GET[stx]%') ";
		break;
		default:
...
$order_query = "order by mn.mn_sort asc";
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_manu as mn where 1 $search_query";
...
if($total){
	...
	$sql = "select * from rb_manu as mn where 1 $search_query $order_query $limit_query";
	$data = sql_list($sql);
