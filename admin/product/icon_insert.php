...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'product/icon_insert.tpl',
	'body'  =>'inc/body.tpl',
	'top'  =>'inc/top.tpl',
...
if($_GET['sca'] && $_GET['stx']){
	switch($_GET['sca']){
		case "bd" : 
			$search_query .= " and (p.pd_name like '%".$_GET['stx']."%' or p.pd_exp like '%".$_GET['stx']."%') ";
		break;
		default:
...
$querys_page[] = "ic_type=".$_GET['ic_type'];
$querys[] = "sca=".$_GET['sca'];
...
if($total){
	...
	$sql = "select p.* from rb_product as p where 1 $search_query $order_query $limit_query";
	$data = sql_list($sql);
