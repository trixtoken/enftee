...
$order_query = "order by q.pq_idx desc";
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_product_qna as q left join rb_product as p on q.pd_idx = p.pd_idx where 1 $search_query";
...
if($total){
	...
	$sql = "select * from rb_product_qna as q left join rb_product as p on q.pd_idx = p.pd_idx where 1 $search_query $order_query $limit_query";
	$data = sql_list($sql);
