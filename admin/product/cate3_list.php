...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'product/cate3_list.tpl',
	'body'  =>'inc/body.tpl',
	'top'  =>'inc/top.tpl',
...
if($_GET['sca'] && $_GET['stx']){
	switch($_GET['sca']){
		case "bd" : 
			$search_query .= " and (p.c3_name like '%".$_GET['stx']."%' or p.c3_contents like '%".$_GET['stx']."%') ";
		break;
		default:
...
if($_GET['c2_idx'] != ""){
	$search_query .= " and c3.c2_idx = '".$_GET['c2_idx']."' ";
}
$querys[] = "c2_idx=".$_GET['c2_idx'];
$querys_page[] = "c2_idx=".$_GET['c2_idx'];

$order_query = "order by c2.c2_sort asc, c3.c3_sort asc";
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_cate3 as c3 where 1 $search_query";
...
if($total){
	...
	$sql = "select * from rb_cate3 as c3 left join rb_cate2 as c2 on c2.c2_idx = c3.c2_idx left join rb_cate1 as c1 on c1.c1_idx = c2.c1_idx where 1 $search_query $order_query $limit_query";
	$data = sql_list($sql);
