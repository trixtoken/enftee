...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'product/cate1_list.tpl',
	'body'  =>'inc/body.tpl',
	'top'  =>'inc/top.tpl',
...
if($_GET['sca'] && $_GET['stx']){
	switch($_GET['sca']){
		case "bd" : 
			$search_query .= " and (p.c1_name like '%".$_GET['stx']."%' or p.c1_contents like '%".$_GET['stx']."%') ";
		break;
		default:
...
$order_query = "order by c1.c1_sort asc";
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_cate1 as c1 where 1 $search_query";
...
if($total){
	...
	$sql = "select * from rb_cate1 as c1 where 1 $search_query $order_query $limit_query";
	$data = sql_list($sql);
