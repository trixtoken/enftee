...
$tpl=new Template;
$tpl->define(array(
    'contents'  =>'member/member_worker_list.tpl',
	'body'  =>'inc/body.tpl',
	'top'  =>'inc/top.tpl',
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_member as m where  mb_status > 0 $search_query";
...
if($total){
	...
	$sql = "select m.* from rb_member as m where mb_status > 0 $search_query $order_query $limit_query";
	$data = sql_list($sql);
