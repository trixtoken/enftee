...
if($_GET['bn_agent']){
	$search_query .= " and m.bn_agent = '".$_GET['bn_agent']."' ";
}
$querys[] = "bn_agent=".$_GET['bn_agent'];
$querys_page[] = "bn_agent=".$_GET['bn_agent'];


$query_order = (is_array($querys) && count($querys) > 0) ? implode("&", $querys) : "";
$order_query = "order by m.bn_idx asc";
if($order_by != ""){
...
// 전체 데이터수 구하기
$sql_total = "select * from rb_banner as m where 1 $search_query";
...
if($total){
	...
	$sql = "select * from rb_banner as m where 1 $search_query $order_query $limit_query";
	$data = sql_list($sql);
